...
def fetch_entity_list_for_kind(kind: str, op: str, cutoff_date: datetime) -> list:
    """
    ...
    try:
        ...
        query.add_filter("created", op, cutoff_date)
...
def fetch_data_for_kind(kind: str, op: str, cutoff_date: datetime) -> list:
    """
        Returns a list of the encrypted data field from each entity within the given kind
    """
    result_list = []
    entities = fetch_entity_list_for_kind(kind, op, cutoff_date)
    for entity in entities:
        result_list.append(entity['encrypted_data'])
    return result_list


def fetch_data_for_survey(survey_id: str, period_list: List[str], op: str, cutoff_date: datetime) -> dict:
    """
    ...
    for period in period_list:
        entities = fetch_entity_list_for_kind(f'{survey_id}_{period}', op, cutoff_date)
        for entity in entities:
